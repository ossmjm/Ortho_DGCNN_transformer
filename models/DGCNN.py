...
class DGCNN(nn.Module):
    def __init__(self, k=20, embed_dim=256, num_dims=13):
        ...
        # Projection to per-tooth features
        self.tooth_proj = nn.Conv2d(embed_dim, embed_dim, kernel_size=(1, 128), stride=(1, 128))  # Reduce points to 4
    
    def forward(self, x):
        logger = logging.getLogger('TrainLogger')
        logger.debug(f"DGCNN input shape={x.shape}")
        
        # Validate input shape: [batch_size, num_teeth, num_points, num_dims]
        if len(x.shape) != 4:
        ...
        # Permute to [batch_size, num_dims, num_teeth, num_points]
        ...
        logger.debug(f"After get_graph_feature 1: shape={x.shape}")
        x = self.conv1(x)  # [batch_size, 64, num_teeth, num_points]
        ...
        # Split into two scales (a=2)
        x = x.view(batch_size, self.embed_dim, 2, num_teeth // 2)  # [batch_size, embed_dim, 2, 7]
        x = x.unsqueeze(-1)  # [batch_size, embed_dim, 2, 7, 1]
        
        # Project to b=4 features per tooth
        x = x.repeat(1, 1, 1, 1, 128)  # [batch_size, embed_dim, 2, 7, 128]
        x = self.tooth_proj(x)  # [batch_size, embed_dim, 2, 7, 4]
        x = x.permute(0, 2, 3, 4, 1).contiguous()  # [batch_size, 2, 7, 4, embed_dim]
        x = x.view(batch_size, 2, num_teeth, 4, self.embed_dim)  # [batch_size, 2, 14, 4, embed_dim]
