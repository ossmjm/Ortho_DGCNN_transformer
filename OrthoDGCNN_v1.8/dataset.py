...
class CumulativeJawTeethDataset(Dataset):
    def __init__(
        ...
        # Remove separate scalers, use custom scaling with predefined ranges
        self.translation_max = 30.0  # Maximum range for translation (mm)
        self.rotation_max = 350.0    # Maximum range for rotation (degrees)
    ...
    def _load_transformations(self, jaw_id, cumulative_df):
        ...
        if cumulative_df is not None:
            for _, row in cumulative_df.iterrows():
                ...
                # Compute directions first (before absolute values)
                if not self.inference:
        ...
        # Apply custom scaling if use_scaler is True, preserving zeros
        if self.use_scaler and not self.inference:
            # Save the position of zeros in a mask
            zero_mask = cumulative_transformations == 0
            # Apply custom scaling to [0, 1] for all parameters
            trans_data = cumulative_transformations[:, :3]  # Translation parameters (0-2)
            rot_data = cumulative_transformations[:, 3:]    # Rotation parameters (3-5)
            scaled_trans_data = trans_data / self.translation_max  # Scale to [0, 1]
            scaled_rot_data = rot_data / self.rotation_max   # Scale to [0, 1]
            cumulative_transformations[:, :3] = scaled_trans_data
            cumulative_transformations[:, 3:] = scaled_rot_data
            # Apply mask again to scaled values to return zeros
            cumulative_transformations = torch.where(zero_mask, torch.zeros_like(cumulative_transformations), cumulative_transformations)
        ...
        if self.inference and self.use_scaler:
            # Save the position of zeros in a mask
            zero_mask = cumulative_transformations == 0
            # Apply inverse scaling to all parameters
            trans_data = cumulative_transformations[:, :3]  # Translation parameters (0-2)
            rot_data = cumulative_transformations[:, 3:]    # Rotation parameters (3-5)
            inverse_scaled_trans_data = trans_data * self.translation_max  # Back to original range
            inverse_scaled_rot_data = rot_data * self.rotation_max   # Back to original range
            cumulative_transformations[:, :3] = inverse_scaled_trans_data
            cumulative_transformations[:, 3:] = inverse_scaled_rot_data
            # Apply mask again to scaled values to return zeros
            cumulative_transformations = torch.where(zero_mask, torch.zeros_like(cumulative_transformations), cumulative_transformations)
    ...
    def _initialize_dataset(self):
        ...
        if not self.inference and self.split == 'train' and self.use_scaler:
            ...
            # No fitting needed for custom scaling, just log ranges
            self.logger.info(f"Using custom scaling ranges: Translation max = {self.translation_max} mm, Rotation max = {self.rotation_max} degrees")
        else:
            self.logger.info(f"Using custom scaling ranges: Translation max = {self.translation_max} mm, Rotation max = {self.rotation_max} degrees for inference")
    ...
    def get_scalers(self):
        """Return the parameter-specific scalers for external use (e.g., inference)."""
        # Return None as custom scaling is used instead of fitted scalers
        return None