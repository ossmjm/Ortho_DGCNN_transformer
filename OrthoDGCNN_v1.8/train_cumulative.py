...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        train_losses = {
            ...
            'active_f1_scores': 0.0
        }
        for batch_idx, batch in enumerate(train_loader):
            jaw_id, point_cloud, cumulative_transforms, _, activity_labels, directions = batch
            ...
            directions = directions.to(device) if directions is not None else None  # (batch_size, num_teeth, 6)
            ...
            outputs = model(
                ...
                active_labels=activity_labels,
                direction_labels=directions,
                training=True,
                epoch=epoch,
                total_epochs=args.epochs,
                val_loss=val_loss_history['total'][-1] if val_loss_history['total'] else None,
                is_freeze=args.is_freeze  # Use args.is_freeze
            )
            trans_mag, rot_mag, active_pred, direction_pred = outputs
            ...
            total_loss, losses = compute_loss(
                ...
                logger=logger,
                args=args,
                is_freeze=args.is_freeze  # Pass is_freeze to compute_loss
            )
        ...
        logger.info(f"Epoch {epoch+1}/{args.epochs}, "
                    ...
                    f"Active: {get_scalar_value(train_losses['loss_active']):.4f}, "
                    ...
                    f"Active_F1: {get_scalar_value(train_losses['active_f1_scores']):.4f})")
        ...
        logger.info(f"Epoch {epoch+1}/{args.epochs}, "
                    ...
                    f"Active: {get_scalar_value(val_losses['loss_active']):.4f}, "
                    ...
                    f"Active_F1: {get_scalar_value(val_losses['active_f1_scores']):.4f})")
    ...
    torch.save(checkpoint, os.path.join(args.output_dir, 'best_model_final.pth'))
    logger.info(f"Saved best full model at epoch {best_epoch} with val_loss {get_scalar_value(best_val_loss):.4f} at the end of training")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_layers', type=int, default=2, help='Number of layers in transformer encoder')
    ...
    parser.add_argument('--w_active', type=float, default=1.0, help='Weight for active loss')
    parser.add_argument('--w_dir', type=float, default=1.0, help='Weight for direction loss')
    ...
    parser.add_argument('--is_freeze', type=str, default='none', choices=['none', 'regression', 'classification'], help='Freeze mode for training (none, regression, classification)')
